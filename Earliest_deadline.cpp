...
bool sort_by_deadline(executed_process a,executed_process b)//sort the processes in ready queue based on deadlines
{
...
void edf_scheduler(vector<task_type>&v,int t,int n)//scheduler that schedules the processes
{
	...
	executed_process extra=executed_process(-99,100000,90000);
	executed_process current;
	current=extra;
	int resumable=0;
	ofstream output,stats,final_output;
	output.open("EDF-Log.txt");
	output<<" Data log for "<<n<<" processe\n";
	while(global_time<t)
	{
		if(ready.size()>0)//this if condition check whether any process in ready queue has missed its deadline
		{
			...
			{
				if(ready[i].deadline==global_time)
				{
					...
					dead[ready[i].ppid]++;
					ready[i].deadline=-100;
					p++;
				}
			}
			sort(ready.begin(),ready.end(),sort_by_deadline);//sorts the ready queue after removing the process that missed their deadline
			while(p>0)
			{
		...
		sort(ready.begin(),ready.end(),sort_by_deadline);//sorts after adding the processses in readdy queue
		//this if condition checks whether the proces sin ready queue has higher priority then the one currently running

		if( (ready.size()>0 && current.deadline>ready[0].deadline && current.till_now!=current.pro_time) ||(ready.size()>0 && current.deadline==ready[0].deadline && current.p_period>ready[0].p_period && current.till_now!=current.pro_time) )
		{
			if(flag_for_idelness==true)
			{
				output<<"idele till t="<<global_time<<"\n";
			...
			sort(ready.begin(),ready.end(),sort_by_deadline);//again sorts the processes in ready queue	
		}
		...
		{
			//takes out the process from the current
			dead[current.ppid]++;
	...
	stats.close();
    final_output.open("Average_time.txt", std::ios_base::app); //outputs to the file
  	final_output<<"\n\nAverage times for EDF scheduling with "<<n<<" processes\n";
...
int main()
{
	...
	//cout<<max;
	edf_scheduler(Tasks,max+10,n);//calls the edf_scheduler function
	return 0;
}